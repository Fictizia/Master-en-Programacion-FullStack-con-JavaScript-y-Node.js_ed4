...
function camarasDGT(camaras) {
    ...
    setTimeout( camarasDGT, 4000, camaras);
...
function TiempoDeHoy(date) {
    ...
    for (var i = 0; i < date.length; i += 36) {
        ...
        const img = tiempo.list[0].weather[0];

        console.log("imagen", img);
        console.log("bucle tiempo2", tiempoFinal);
        console.log("mi viento", vientoFinal);

        crearDatosTiempo(tiempoFinal.temp, tiempoFinal.temp_min, tiempoFinal.temp_max, tiempoFinal.humidity, tiempoFinal.pressure, vientoFinal.speed, descripcion.description, img.icon);

    }

}

function crearDatosTiempo(temp, temp_min, temp_max, humidity, pressure, speed, description, icon) {
...
function tiempoFuturo(date) {
    ...
    for (var i = 8; i < date.length; i += 4) {
        ...
        const img = nextTiempo.list[i].weather[0];
        const dia = nextTiempo.list[i];
        crearDatosTiempo2(tiempoFinal.temp, tiempoFinal.temp_min, tiempoFinal.temp_max, tiempoFinal.humidity, tiempoFinal.pressure, vientoFinal.speed, descripcion.description, dia.dt_txt, img.icon);


    }
}

function crearDatosTiempo2(temp, temp_min, temp_max, humidity, pressure, speed, description, dt_txt, icon) {
    var url = "http://openweathermap.org/img/wn/" + icon + "@2x.png";
    var article = document.createElement("article");
    var h2 = document.createElement("h2");
    var img = document.createElement("img");
